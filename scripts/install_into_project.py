...
What it does:
    1. Removes the files listed in <target>/.cursor/TEMPLATE_MANIFEST (what the previous
       run installed) and rewrites them from apm_config/, commands/, hooks/, hooks.json,
       rules/, skills/ — with bilingual "cs:" comments stripped. Files NOT in the
       manifest (project rules rules/9xx-*.mdc, own skills/commands, Cursor's mcp.json)
       are left untouched.
    2. Writes <target>/.cursor/TEMPLATE_VERSION (source version + timestamp) and a fresh
       TEMPLATE_MANIFEST.
...
Hard rule this script enforces: template-owned files under .cursor/ are 100% generated.
Config overrides belong in doc/apm_config/*.user.md; project-owned rules use the 9xx-
namespace — see rules/200-project-design-rules.mdc.
...
from lib.installer import LEGACY_VERSION, TemplateInstaller, TemplateInstallError  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print()
    print("Done. Remember:")
    print(f"  - Files listed in {target / '.cursor' / 'TEMPLATE_MANIFEST'} are generated —")
    print("    never hand-edit them; they are rewritten on every re-install.")
    print(f"  - Configure this project via {target / 'doc' / 'apm_config'}/*.user.md")
    print("  - Project-specific rules: .cursor/rules/9xx-<slug>.mdc (survive re-install).")
    return 0
